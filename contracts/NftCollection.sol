...
contract NftCollection is Initializable, Context, ERC165, ERC2981, IERC1155, IERC1155MetadataURI {
    ...
    // FIXME: change this to above events
    ...
    function _mint(address to, uint256 id, uint256 amount, string memory uri_, bytes memory data) internal virtual {
        ...
        _balances[supply][to] += amount;
        _setTokenURI(supply, uri_);
        supply++;
    ...
    function _mintBatch(address to, uint256[] memory ids, string[] memory uris) internal virtual {
        ...
        for (uint256 i = 0; i < idsLength; ) {
            _balances[supply][to] += 1;
            _setTokenURI(supply, uris[i]); // this increases gas cost, will implement better alternative
            realIds[i] = supply;
            supply++;
    ...
    function _burnBatch(address from, uint256[] memory ids, uint256[] memory amounts) internal virtual {
        ...
        emit TransferBatch(operator, from, address(0), ids, ids);
    }
