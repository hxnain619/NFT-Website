...
import {
    EVENT_ACCEPT_OFFER, EVENT_AUCTION_CANCELED, EVENT_AUCTION_CREATED, EVENT_AUCTION_FINALIZED, EVENT_BID_SUCCESS,
    EVENT_BUYING_LISTING,
    EVENT_CANCEL_LISTING, EVENT_CANCELED_OFFER,
    EVENT_CHANGED_LISTING,
    EVENT_CREATED_LISTING, EVENT_CREATED_OFFER, EVENT_TRANSFER
...
export default function useHandleSocket() {
    ...
    const messageHandler = () => {
        ...
        const currentToken = token?.list?.find((item) => item?.contractAddress?.toLowerCase() === data?.nftAddress?.toLowerCase() && item?.tokenId?.toString() === data?.tokenID)

        if (data?.confirmed) {
            switch (message?.action) {
                case EVENT_CREATED_LISTING: {
                    if (location.pathname?.includes(`${currentToken?.contractAddress}/${currentToken?.tokenId}`)
